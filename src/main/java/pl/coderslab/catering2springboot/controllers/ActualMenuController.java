...
public class ActualMenuController {
    ...
    public ActualMenuController(ActualMenuRepository actualMenuRepository, ActualOrderRepository actualOrderRepository, NewMenuRepository newMenuRepository, NewOrderRepository newOrderRepository) {
...
//        if (session.getAttribute("userId") != null && (Boolean) session.getAttribute("superAdmin")) {
        ...
        for (NewMenu dinner : allDinners) {
            ...
            for (ActualOrder newOrder : allNewOrders) {
                if (dinner.getMealNo().equals(newOrder.getMealMon()) || dinner.getMealNo().equals(newOrder.getMealTue()) || dinner.getMealNo().equals(newOrder.getMealWed()) || dinner.getMealNo().equals(newOrder.getMealThu()) || dinner.getMealNo().equals(newOrder.getMealFri())) {
                    if (newOrder.getShiftMon() == 1 || newOrder.getShiftTue() == 1 || newOrder.getShiftWed() == 1 || newOrder.getShiftThu() == 1 || newOrder.getShiftFri() == 1) {
                        idsFirstShift = idsFirstShift + newOrder.getUser().getUserId() + ", ";
                        idsFirstShiftQty++;
                    } else if (newOrder.getShiftMon() == 2 || newOrder.getShiftTue() == 2 || newOrder.getShiftWed() == 2 || newOrder.getShiftThu() == 2 || newOrder.getShiftFri() == 2) {
